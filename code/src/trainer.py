...
class LAMCNNTrainer(SimpleCNNTrainer):
    def __init__(
        self,
        model: nn.Module,
        optimizer: Optimizer,
        ...
        super().__init__(model, optimizer, criterion, verbose_period, device, transform)
        self.hyperparameter = hyperparameter

    def ss_pairing(self, x, y):
        """Labeled SS pairing (where augmented samples are generated by startified shuffling)."""
        unique_labels = torch.unique(y)
        new_x = x.clone()
        for c in unique_labels:
            _idx = (y == c).nonzero(as_tuple=True)[0]
            _perm = torch.randperm(_idx.shape[0])  # shuffle in the stratum
            new_x[_idx] = x[_idx[_perm]]
        return new_x
    ...
    def _train(self, dataloader: DataLoader, verbose: bool, epoch_id: int):
        ...
        with tqdm(dataloader, unit="batch", disable=not verbose) as bar:
            ...
            for batch in bar:
                ...
                if self.transform:
                    X_batch = self.transform(X_batch)
                X_tilde_batch = self.ss_pairing(X_batch, y_batch)
                optimizer.zero_grad()
                logits = cnn(X_batch)
                loss_ce = criterion(logits, y_batch)
                loss_lam = lam_loss(
                    cnn.net(X_batch),
                    cnn.net(X_tilde_batch),
                    y_batch,
                    cnn.cls_head.weight,
                )
                loss = loss_ce + self.hyperparameter["lam_coef"] * loss_lam
                loss.backward()
                optimizer.step()

                # update running stats
                bar.set_postfix(ce_loss=float(loss_ce), lam_loss=float(loss_lam))
    ...
    def evaluate(self, dataloader, verbose, epoch_id):
        ...
        with torch.no_grad():
            for batch in tqdm(
                ...
                total_kl_c += _kl_c
                total_kl_s += _kl_s
                total_c_loss += _c_loss
                total_s_loss += _s_loss
